...
class GridGraph:
    ...
    def cell_to_idx(self, i, j):
        """Converts a cell coordinate to a flattened index."""
        return i + j * self.width

    def idx_to_cell(self, idx):
        """Converts a flattened index back to a Cell."""
        i = idx % self.width
        j = idx // self.width
        return Cell(i, j)
    ...
    def get_parent(self, cell):
        ...
        if self.parents is None or self.width <= 0 or self.height <= 0:
            return None

        idx = self.cell_to_idx(cell.i, cell.j)
        ...
        return self.idx_to_cell(parent_idx)
    ...
    def init_graph(self):
        ...
        self.parents = [-1 for _ in range(num_cells)]
        self.g_cost = [np.inf for _ in range(num_cells)]
        self.f_score = [np.inf for _ in range(num_cells)]
        self.expanded = [False for _ in range(num_cells)]
    ...
    def find_neighbors(self, i, j):
        ...
        for di, dj in neighbor_offsets:
            ...
            if self.is_cell_in_bounds(ni, nj):
                nbrs.append(self.cell_to_idx(ni, nj))
